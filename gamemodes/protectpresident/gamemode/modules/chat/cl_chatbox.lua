--[[
	...
	Credits to HatsChat for creating the chat.
	Credits to Blt950 for custom edit and themes.
...
local InputText = "" //The input text
...
local Enabled = true //For the toggle command
...
function chat.AddText( ... )
	...
	for k,v in pairs( args ) do
		if (type(v) != "string") and !((type(v) == "table") and (v.r and v.g and v.b)) then //Unusual entry
			if (type(v) == "Player") then //It's a payer, use their name and team colour
...
//"Other" messages
hook.Add("ChatText", "HatsChat_ChatText", function(plindex, plname, text, typ)
...
local FGCol = Color(25, 25, 25, 101) //Foreground (Scrollbar, text background)
...
local function DrawChat()
	if Enabled then
		...
		if ShowChat then
			...
			surface.SetFont("InputText")
			local InputVerf = string.gsub( InputText, "&", "#" ) //& says length 0, # should be aprox the right size
			...
			//
			draw.RoundedBox(0, x-17, y+4, 90, 20, Color( 25, 25, 25, 200 ))

			surface.SetDrawColor( 25, 25, 25, 255 )
			draw.RoundedBox(0, x+73, y+4, ScrW()/3.5+1, 20, Color( 25, 25, 25, 225 )) // 779 and 778

			surface.SetDrawColor( 255, 255, 255, 255 )
			draw.RoundedBox(0, x+73, y+5, ScrW()/3.5, 18, Color( 255, 255, 255, 225 ))
			//
			
			//Chat prompt text
			if TeamChat then
				draw.SimpleText( "Team: ", "ChatText", x+70, y+5, Color(255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT )
			else
				draw.SimpleText( chattype, "ChatText", x+70, y+5, Color(255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT )
			end
			...
			if InputSel.Select then
				...
				local InputVerf = string.gsub( InputText, "&", "#" )
				local Start, End
				...
				draw.RoundedBox(2, x+80+StrStart, y+6, Width, 18, HCol)
			end
			
			//Input Text (The important bit)
			draw.SimpleText( InputText , "InputText", x+80, y+5, BorderCol, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT )
			
			//Cursor
			if (cur.Show) then
				draw.RoundedBox(0, x+80+cur.Pos, y+5, 1, 17, BorderCol)
			end
...
local function AddText( str ) //To the input box
	...
	if InputSel.Select then //Something is selected. We're replacing it.
		...
		Start,End = string.sub(InputText,0,math.max(NumStart,0)),string.sub(InputText,NumEnd+1,#InputText+1)
		
		InputSel.Select = false //Update boolean
		
		cur.At = NumStart //Update cursor position
	else //Nothing is selected
		Start, End = string.sub(InputText,1,cur.At),string.sub(InputText,cur.At+1,-1)
	end
	
	if cur.At == #InputText then End = "" end //Fix End if we're at the end
	
	if #(Start..str..End) <=126 then //We're within limits
		InputText = Start..str..End //Insert new string
		
		cur.At = math.min(cur.At + #str, #InputText) //Update cursor
		...
		if #(Start..End) < 126 then //The string we already have isn't hitting the limit
			...
			for i=1,#tab do
				...
				if #(Start..char..End)<=126 then //It fits
					...
					InputText = Start..End //Add the end
					break //Finish
				end
			end
		end
	...
	hook.Call("ChatTextChanged", GAMEMODE, tostring(InputText) ) //Allow other scripts to interact
end
...
local function KeyPress()
	if ShowChat and Enabled then
		...
		elseif (input.IsKeyDown(KEY_C) and input.IsKeyDown(KEY_LCONTROL)) and ((!Pressed[KEY_C]) or (HoldKey[1]==KEY_C and HoldKey[2]<CurTime())) and InputSel.Select then
			if InputSel.Start < cur.At then
				SetClipboardText( string.sub( InputText, InputSel.Start+1, cur.At ) )
			elseif InputSel.Start ~= cur.At then
				SetClipboardText( string.sub( InputText, cur.At+1, InputSel.Start ) )
			else
		...
		elseif input.IsKeyDown(KEY_LEFT) and ((!Pressed[KEY_LEFT]) or (HoldKey[1]==KEY_LEFT and HoldKey[2]<CurTime())) then
			...
			if input.IsKeyDown(KEY_LCONTROL) then
				...
				for i=(cur.At-1),0,-1 do
					if string.sub(InputText,i,i) == " " then
						num=i
						break
					end
				end
		...
		elseif input.IsKeyDown(KEY_RIGHT) and ((!Pressed[KEY_RIGHT]) or (HoldKey[1]==KEY_RIGHT and HoldKey[2]<CurTime())) then
			...
			if input.IsKeyDown(KEY_LCONTROL) then
				...
				cur.At = num
				if cur.At>#InputText then cur.At = #InputText end
				if cur.At==0 then cur.Pos = 0 else
					surface.SetFont("InputText")
					local InputVerf = string.gsub( InputText, "&", "#" )
					cur.Pos = surface.GetTextSize( string.sub(InputVerf, 1, cur.At) )
				...
				cur.At = math.min(#InputText, cur.At+1)
				if cur.At==0 then cur.Pos = 0 else
					surface.SetFont("InputText")
					local InputVerf = string.gsub( InputText, "&", "#" )
					cur.Pos = surface.GetTextSize( string.sub(InputVerf, 1, cur.At) )
		...
		elseif input.IsKeyDown(KEY_BACKSPACE) and ((!Pressed[KEY_BACKSPACE]) or (HoldKey[1]==KEY_BACKSPACE and HoldKey[2]<CurTime())) then
			...
			if InputSel.Select then
				...
				StrStart,StrEnd = string.sub(InputText,0,math.max(Start,0)),string.sub(InputText,End+1,#InputText+1)
				InputText = StrStart..StrEnd
				...
				local Start, End = string.sub(InputText,0,math.max(cur.At-1,0)),string.sub(InputText,cur.At+1,-1)
				InputText = Start..End
		...
		elseif input.IsKeyDown(KEY_DELETE) and ((!Pressed[KEY_DELETE]) or (HoldKey[1]==KEY_DELETE and HoldKey[2]<CurTime())) then
			...
			if InputSel.Select then
				...
				if cur.At+2 >#InputText then InputText=string.sub(InputText,1,cur.At) else
					local Start, End = string.sub(InputText,1,cur.At),string.sub(InputText,cur.At+2,-1)
					InputText = Start..End
				end
				
				cur.At = math.min(cur.At,#InputText)
				if cur.At==0 then cur.Pos = 0 else
					surface.SetFont("InputText")
					local InputVerf = string.gsub( InputText, "&", "#" )
					cur.Pos = surface.GetTextSize( string.sub(InputVerf, 1, cur.At) )
			...
			if not PasteFrame then
				...
				PasteBox.OnEnter = function(self)
					...
					if #InputText > 0 then
						if TeamChat then
							RunConsoleCommand("say_team", InputText)
						else
							RunConsoleCommand("say", InputText)
						end
					end
					CloseChat()
				end
...
local function Think()
	...
	if input.IsMouseDown( MOUSE_LEFT ) then
		...
		//Highlighting Input
		if InputHigh then
			local tab = string.ToTable( InputText )
			local str = ""
			local size = 0
			cur.At = 0
...
concommand.Add("HatsChat_Test", function(ply, c, a)
	...
	timer.Create("ChatTextTesting",0, loop, function()
...
hook.Add("StartChat", "StartChat", function() RunConsoleCommand("ptp_StartChat") end)
hook.Add("FinishChat", "FinishChat", function() RunConsoleCommand("ptp_StopChat") chattype = "Chat: " end)

hook.Add("ChatTextChanged", "ptp_TextChanged", function(text) 
	local overhead
	chatmessage = text

	if (string.sub(text, 1, string.len(chatprefix) + 2) == chatprefix.."/ ") or (string.sub(text, 1, string.len(chatprefix) + 4) == chatprefix.."ooc ") then
		chattype = "OOC: "
		overhead = "Typing.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 2) == chatprefix.."r ") or (string.sub(text, 1, string.len(chatprefix) + 6) == chatprefix.."radio ") then
		chattype = "Radio: "
		overhead = "Radioing.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 3) == chatprefix.."me ") then
		chattype = "Perform: "
		overhead = "Performing.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 2) == chatprefix.."y ") then
		chattype = "Yell: "
		overhead = "Yelling.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 2) == chatprefix.."w ") then
		chattype = "Whisper: "
		overhead = "Whispering.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 3) == chatprefix.."pm ") then
		chattype = "PM: "
		overhead = "Messaging.."
	elseif (string.sub(text, 1, string.len(chatprefix) + 10) == chatprefix.."broadcast ") then
		chattype = "Broadcast: "
		overhead = "Announcing.."
	else
		chattype = "Chat: "
	...
	net.Start( "ptp_MyChat" )
		net.WriteEntity(LocalPlayer())
		net.WriteString( overhead )
	net.SendToServer()
 end)
...
hook.Add("PostPlayerDraw", "DrawTyping", function(ply)
	if ply:GetSharedVar("istyping") then
		local alpha = CalculateAlphaFromDistance(300, LocalPlayer(), ply);
		local eyeAngles = LocalPlayer():EyeAngles()
		local position = ply:GetPos()

		if ply:Crouching() then
			position = position + Vector(0,0,64)
		else 
			position = position + Vector(0,0,80)
		end

		cam.Start3D2D(position, Angle(0, eyeAngles.y-90 , 90), 0.04);
			draw.RoundedBox(5, -300, 0, 600, 120, Color( 25, 25, 25, alpha/2.5 ))
			draw.DrawText(ply:GetSharedVar("typing") or "", "ptp_TypingFont", 0, 0, Color(255, 255, 255, alpha), TEXT_ALIGN_CENTER )
		cam.End3D2D();
	end
end)